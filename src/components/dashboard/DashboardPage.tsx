...
export function DashboardPage({ domain, dispatch }: { domain: DomainState, dispatch: (e: AppEvent) => void }) {
  ...
  return (
    <div className="panel animate-page-enter">
      {/* Hero Section - Total Loss with Stamp */}
      <div className="hero-section" style={{ background: 'var(--colorNeutralBackground4)', border: '2px solid var(--colorStatusDangerForeground1)', marginBottom: 16 }}>
        {/* Audit Stamp */}
        <div className="hero-stamp animate-stamp">
          <div className={`audit-stamp ${stampType}`}>
            {stampLabel}
          </div>
        </div>

        <div className="hint mono-font" style={{ textTransform: 'uppercase', letterSpacing: '0.15em', fontSize: 'var(--fontSizeBase100)' }}>현재까지 총 손실</div>
        <div className="hero-value danger" style={{ marginTop: 8 }}>
          -₩{report.totals.netLossWon.toLocaleString()}
        </div>
        <div className="hint mono-font" style={{ marginTop: 12 }}>
          시간 {totalHours}h · 직접 비용 ₩{totalDirectMoney.toLocaleString()} · ROI {report.totals.roiPct}%
        </div>
        {report.topCauseLabel && (
          <div style={{ marginTop: 12 }}>
            <span className="pillMini danger">손해 1위</span>
            <span style={{ marginLeft: 8, fontWeight: 700 }}>{report.topPersonLabel}</span>
            <span className="muted" style={{ marginLeft: 8 }}>원인: {report.topCauseLabel}</span>
          </div>
        )}
      </div>
      ...
      <div className="row" style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div className="h1">관계 감사 리포트</div>
          <div className="hint">사람/상황을 기록하면, 손익과 원인이 자동으로 뽑힌다.</div>
        </div>
        <div>
          <div className="row" style={{ gap: 8, flexWrap: 'wrap' }}>
            <input className="input small" placeholder="YYYY-MM" value={month} onChange={(e) => handleMonthChange(e.currentTarget.value)} style={{ width: 100 }} />
            <select className="input small" value={filterPersonId} onChange={(e) => setFilterPersonId(e.currentTarget.value)} style={{ width: 100 }}>
              <option value="">모든 사람</option>
              {domain.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <select className="input small" value={filterCause} onChange={(e) => setFilterCause(e.currentTarget.value as CauseKey | '')} style={{ width: 110 }}>
            ...
            <Button onClick={() => { setMonth(''); setMonthError(''); setFilterPersonId(''); setFilterCause(''); }}>초기화</Button>
          </div>
          {monthError && <div className="hint danger" style={{ marginTop: 4 }}>{monthError}</div>}
        </div>
      </div>

      {/* B2B Filter Tabs */}
      <TabList
        selectedValue={viewFilter}
        onTabSelect={(_, data) => setViewFilter(data.value as ViewFilter)}
        style={{ marginTop: 12 }}
      >
        <Tab value="all">전체 ({domain.people.length})</Tab>
        <Tab value="personal">관계 ({personalCount})</Tab>
        <Tab value="client">클라이언트 ({clientCount})</Tab>
      </TabList>
      ...
      <div className="grid cols-2" style={{ marginTop: 14 }}>
        <Card className="card">
          <div className="h2">대상 추가</div>
          <div className="row" style={{ marginTop: 10 }}>
            <input className="input" placeholder="이름/호칭 (공유 시 익명화 가능)" value={newPersonName} onChange={(e) => { setNewPersonName(e.currentTarget.value); setPersonNameError(''); }} />
            <Button appearance="primary" onClick={addPerson}>추가</Button>
          </div>
          <div className="row" style={{ marginTop: 8 }}>
            <label className="row" style={{ gap: 8 }}>
              <input type="checkbox" checked={isClientNew} onChange={(e) => setIsClientNew(e.currentTarget.checked)} />
              <span style={{ fontWeight: 600 }}>클라이언트 (업무)</span>
            </label>
          </div>
          {personNameError && <div className="hint danger" style={{ marginTop: 6 }}>{personNameError}</div>}
          ...
          <div className="list" style={{ marginTop: 10 }}>
            {filteredPeople.length === 0 && <div className="hint">비어있음. 사람 1명 추가하고 '오늘 10초 기록'부터.</div>}
            {filteredPeople.map(p => (
              <div key={p.id} className="listItem" style={{ cursor: 'pointer' }} onClick={() => setDetailPerson(p)}>
                <div>
                  <div style={{ fontWeight: 800 }}>
                    {p.name}
                    {p.isClient && <span className="pillMini" style={{ marginLeft: 6 }}>업무</span>}
                  </div>
                  <div className="hint">기록: {domain.entries.filter(e => e.personId === p.id).length}개</div>
                </div>
                <div className="row" onClick={(e) => e.stopPropagation()}>
                  <Button onClick={() => setEntryPersonId(p.id)}>선택</Button>
                  <Button appearance="subtle" onClick={() => setDetailPerson(p)}>상세</Button>
                  <Button appearance="subtle" onClick={() => removePerson(p.id)} style={{ color: 'var(--colorStatusDangerForeground1)' }}>삭제</Button>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card className="card">
          <div className="h2">기록 추가</div>
          <div className="hint">기분/상호성/경계침해가 핵심. 여기서 "손해 이유"가 갈린다.</div>

          <div className="grid" style={{ marginTop: 12 }}>
            <div className="row">
              <select value={entryPersonId} onChange={(e) => setEntryPersonId(e.currentTarget.value)}>
                <option value="">사람 선택</option>
                {domain.people.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
              <input className="input" type="date" value={entryDate} onChange={(e) => setEntryDate(e.currentTarget.value)} />
            </div>
            ...
            <div className="row">
              ...
              />
              <div className="hint">분(시간 비용으로 환산)</div>
              <input
                className="input small"
                type="number"
                min={0}
              ...
              />
              <div className="hint">원(직접 지출)</div>
            </div>

            <div className="row">
              <label className="pill">
                기분 변화
                <select value={moodDelta} onChange={(e) => setMoodDelta(Number(e.currentTarget.value) as -2 | -1 | 0 | 1 | 2)}>
              ...
              <label className="pill">
                상호성
                <select value={reciprocity} onChange={(e) => setReciprocity(Number(e.currentTarget.value) as 1 | 2 | 3 | 4 | 5)}>
            ...
            <div className="row">
              <label className="row" style={{ gap: 8 }}>
                <input type="checkbox" checked={boundaryHit} onChange={(e) => setBoundaryHit(e.currentTarget.checked)} />
                <span style={{ fontWeight: 800 }}>경계 침해 있었음</span>
              </label>
            </div>

            <textarea className="textarea" placeholder="메모 (공유 전 PII 스캔됨)" value={note} onChange={(e) => { setNote(e.currentTarget.value); setEntryError(''); }} />

            {entryError && <div className="hint danger" style={{ marginBottom: 8 }}>{entryError}</div>}
            <Button appearance="primary" onClick={addEntry} disabled={!entryPersonId}>기록 저장</Button>
          </div>
        </Card>
      </div>
      ...
      <div className="grid cols-2" style={{ marginTop: 14 }}>
        <Card className="card">
          ...
          <div className="hint">핵심만. 숫자 보고 감정 끊고 판단해.</div>

          <div className="stats" style={{ marginTop: 10 }}>
            <div className="stat">
              <div className="hint">총 손실</div>
              <div className="big danger">₩{report.totals.netLossWon.toLocaleString()}</div>
            </div>
            <div className="stat">
              <div className="hint">총 비용</div>
              <div className="big">₩{report.totals.costWon.toLocaleString()}</div>
            </div>
            <div className="stat">
              <div className="hint">ROI</div>
              <div className={`big ${report.totals.roiPct < 0 ? 'danger' : ''}`}>{report.totals.roiPct}%</div>
            </div>
          </div>

          <div className="callout" style={{ marginTop: 12 }}>
            <div style={{ fontWeight: 900 }}>원인 1위: <span style={{ color: 'var(--accent)' }}>{report.topCauseLabel}</span></div>
            <div className="hint">손해 제일 큰 사람: {report.topPersonLabel}</div>
          </div>

          <div className="h2" style={{ marginTop: 16 }}>사람별 (손해순)</div>
          <div className="table" style={{ marginTop: 10 }}>
            <div className="thead">
              <div>상대</div><div>손실</div><div>원인</div><div>ROI</div>
            </div>
            {report.people.length === 0 && <div className="hint" style={{ padding: 10 }}>비어있음. 기록 2개만 쌓이면 여기부터 재미있다.</div>}
            {report.people.map(p => (
              <div key={p.personId} className="trow">
                <div style={{ fontWeight: 800 }}>{p.personName}</div>
                <div className="danger">-₩{p.netLossWon.toLocaleString()}</div>
                <div className="pillMini">{causeLabel(p.topCause)}</div>
                <div>{p.roiPct}%</div>
              </div>
            ))}
          </div>
        ...
        <Card className="card">
          ...
          <div className="hint">삭제 가능. (사람 삭제하면 해당 기록도 같이 삭제)</div>

          <div className="list" style={{ marginTop: 10, maxHeight: 520, overflow: 'auto' }}>
            {domain.entries.length === 0 && <div className="hint">비어있음. 위에서 '오늘 10초 기록' 한 번만 찍어.</div>}
            {domain.entries.slice(0, 30).map(e => {
              ...
              return (
                <div key={e.id} className="listItem" style={{ alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 900 }}>{person?.name || '(unknown)'} <span className="hint">· {e.date}</span></div>
                    <div className="hint">{e.minutes}분 · ₩{e.moneyWon.toLocaleString()} · 기분 {e.moodDelta} · 상호성 {e.reciprocity} · {e.boundaryHit ? '경계침해' : '—'}</div>
                    {e.note && <div className="note">{e.note}</div>}
                  </div>
                  <div className="row">
                    <Button onClick={() => setEditingEntry(e)}>수정</Button>
                    <Button appearance="subtle" onClick={() => removeEntry(e.id)} style={{ color: 'var(--colorStatusDangerForeground1)' }}>삭제</Button>
                  </div>
                </div>
              )
            })}
