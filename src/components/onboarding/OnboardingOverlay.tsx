...
import { Button, Card, Dialog, DialogSurface, DialogBody } from '@fluentui/react-components'
...
export function OnboardingOverlay({ domain, dispatch }: { domain: DomainState; dispatch: (e: AppEvent) => void }) {
  ...
  const StepDots = () => (
    <div className="obDots">
      {[0,1,2,3,4,5].map(i => (
        <div key={i} className={`obDot ${i<=step ? 'on' : ''}`}></div>
      ))}
    </div>
  )
  ...
  return (
    <Dialog open>
      <DialogSurface className="modal obModal">
        <DialogBody>
          <div className="row" style={{ justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div className="badge">1분 온보딩</div>
              <StepDots />
            </div>
          ...
          {step === 0 && (
            <div style={{ marginTop: 14 }}>
              ...
              {/* 10초 체험 CTA */}
              <div className="callout" style={{ marginTop: 14, background: 'rgba(124, 58, 237, 0.15)', border: '1px solid rgba(124, 58, 237, 0.3)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontWeight: 900, fontSize: 16 }}>⚡ 10초 체험</div>
                    <div className="hint" style={{ marginTop: 4 }}>샘플 데이터로 공유 카드까지 바로 확인</div>
                  </div>
                  <Button appearance="primary" style={{ whiteSpace: 'nowrap' }} onClick={quickDemo}>
                    바로 공유 카드 보기
                  </Button>
                </div>
              </div>
              ...
              <Card className="card" style={{ marginTop: 14 }}>
                <div className="h2">규칙</div>
                <ul className="obList">
                  <li>이 앱은 너 대신 착한 척 안 한다. <span className="muted">손해면 손해라고 말한다.</span></li>
                  <li>데이터는 브라우저 로컬에만 저장. <span className="muted">기본은 서버 전송 없음.</span></li>
                  <li>공유할 땐 A/B/C 익명화 + 체크리스트로 사고 방지.</li>
                </ul>
              </Card>

              <div className="row" style={{ justifyContent: 'space-between', marginTop: 14 }}>
                <div className="hint">또는 직접 설정하면서 시작 →</div>
                <Button appearance="primary" onClick={next}>시작</Button>
              </div>
            </div>
          )}
          ...
          {step === 1 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div className="grid cols-2" style={{ marginTop: 14 }}>
                <Card className="card">
                  <div className="h2">시급 선택</div>
                  <div className="hint">기준을 정해야 손해가 "숫자"로 보인다.</div>
                  <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
                    {HOURLY_PRESETS.map(p => (
                      <Button
                        key={p.value}
                        appearance={hourlyRate === p.value ? 'primary' : 'secondary'}
                        style={{ justifyContent: 'space-between', width: '100%' }}
                        onClick={() => { setHourlyRate(p.value); setCustomRate('') }}
                      >
                        <span>{p.label}</span>
                        <span className="muted">₩{p.value.toLocaleString()}/h</span>
                      </Button>
                    ))}
                  </div>
                ...
                <Card className="card">
                  <div className="h2">예시 계산</div>
                  <div className="hint">30분 통화 = 얼마?</div>
                  <div className="obPreview" style={{ marginTop: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>30분 × ₩{hourlyRate.toLocaleString()}/h</span>
                      <span className="big danger">-₩{Math.round((30/60) * hourlyRate).toLocaleString()}</span>
                    </div>
                    <div className="hr"></div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>1시간 × ₩{hourlyRate.toLocaleString()}/h</span>
                      <span className="big danger">-₩{hourlyRate.toLocaleString()}</span>
                    </div>
                    <div className="hr"></div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span>2시간 × ₩{hourlyRate.toLocaleString()}/h</span>
                      <span className="big danger">-₩{(hourlyRate * 2).toLocaleString()}</span>
                    </div>
                  </div>
              ...
              <div className="row" style={{ justifyContent: 'space-between', marginTop: 14 }}>
                ...
                <Button appearance="primary" disabled={hourlyRate <= 0} onClick={next}>
          ...
          {step === 2 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div className="grid cols-2" style={{ marginTop: 14 }}>
                <Card className="card">
                  <div className="h2">현재 등록</div>
                  <div className="obPeople">
                    {people.length === 0 && <div className="hint">아직 없음</div>}
                    {people.map(p => <span key={p.id} className="obChip">{p.name}</span>)}
                  </div>
                  ...
                  <div className="row" style={{ marginTop: 12, gap: 8 }}>
                    ...
                    />
                    <Button onClick={() => { addPerson(name); setName('') }}>추가</Button>
                  </div>

                  <div className="row" style={{ marginTop: 10, justifyContent: 'space-between' }}>
                    <Button onClick={quickPeople}>빠른 시작 (A/B 자동)</Button>
                    <div className="pill">{people.length}/2</div>
                  </div>
                </Card>

                <Card className="card">
                  <div className="h2">왜 2명이냐?</div>
                  <div className="hint">비교가 생기면 뇌가 "패턴"을 인식한다. 그때부터 재미있어진다.</div>
                  <div style={{ marginTop: 10 }}>
                    <div className="pill warn">팁</div>
              ...
              <div className="row" style={{ justifyContent: 'space-between', marginTop: 14 }}>
                ...
                <Button appearance="primary" disabled={people.length < 2} onClick={next}>
                  다음
                </Button>
              </div>
            </div>
          ...
          {step === 3 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div className="grid cols-2" style={{ marginTop: 14 }}>
                <Card className="card">
                  <div className="h2">샘플 기록 자동 생성</div>
                  <div className="hint">"경계 침해 + 후회" 1개, "상호 이득" 1개를 넣어 비교를 만든다.</div>
                  <div className="row" style={{ marginTop: 12, justifyContent: 'space-between' }}>
                    <Button disabled={entries.some(e => (e.note||'').includes('[demo]'))} onClick={seedDemo}>
                      샘플 추가
                    </Button>
                    <div className="pill">{entries.length}개 기록</div>
                  </div>
                  <div className="hint" style={{ marginTop: 10 }}>
                    샘플은 note에 <span className="mono">[demo]</span>가 붙는다. 싫으면 나중에 삭제하면 끝.
                  </div>
                </Card>

                <Card className="card">
                  <div className="h2">직접 기록할래?</div>
                  <div className="hint">대시보드에서 1분이면 된다. 딱 이것만:</div>
                  <ul className="obList">
                    <li>몇 분 뺏겼나</li>
                    <li>돈 나갔나</li>
                    <li>기분이 내려갔나</li>
                  ...
                  <div className="row" style={{ marginTop: 10 }}>
                    <Button onClick={() => dispatch({ type: 'SET_TAB', tab: 'dashboard' })}>대시보드로 가기</Button>
                  </div>
                </Card>
              </div>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div className="grid cols-2" style={{ marginTop: 14 }}>
                <Card className="card">
                  ...
                  {!report && <div className="hint">리포트를 만들 수 없음</div>}
                  {report && (
                    <div className="obPreview">
                      ...
                      <div className="obKpis">
                        <div className="obKpi">
                          <div className="hint">최악의 사람</div>
                          <div style={{ fontWeight: 800 }}>{report.topPersonLabel}</div>
                        </div>
                        <div className="obKpi">
                          <div className="hint">주 원인</div>
                          <div style={{ fontWeight: 800 }}>{report.topCauseLabel}</div>
                        </div>
                      </div>
                      <div className="hint" style={{ marginTop: 10 }}>공유 탭에서 레이아웃/카피를 더 잔인하게 바꿀 수 있다.</div>
                    </div>
                  )}
                </Card>

                <Card className="card">
                  <div className="h2">다음 액션</div>
                  <div className="hint">스토리 올리는 건 선택. 그래도 "정리"는 남는다.</div>
                  <ul className="obList">
                  ...
                  <div className="row" style={{ marginTop: 12, gap: 8 }}>
                    <Button onClick={() => dispatch({ type: 'SET_TAB', tab: 'share' })}>공유 탭 열기</Button>
                    <Button onClick={() => dispatch({ type: 'SET_TAB', tab: 'coach' })}>코치 열기</Button>
                  </div>
                </Card>
              </div>
              ...
              <div className="row" style={{ justifyContent: 'space-between', marginTop: 14 }}>
                ...
                <Button appearance="primary" onClick={() => setStep(5)}>다음</Button>
              </div>
            </div>
          )}
          ...
          {step === 5 && (
            <div style={{ marginTop: 14 }}>
              ...
              <div className="grid cols-3" style={{ marginTop: 14, gap: 12 }}>
                <Card
                  className="card"
                ...
                >
                  ...
                  <div className="h2" style={{ margin: 0 }}>기록 추가</div>
                  <div className="hint" style={{ marginTop: 4 }}>10초면 된다. 시간/돈/감정만 찍어.</div>
                  <div className="pill" style={{ marginTop: 8 }}>추천</div>
                </Card>
                ...
                >
                  ...
                  <div className="h2" style={{ margin: 0 }}>코치 판결</div>
                  <div className="hint" style={{ marginTop: 4 }}>거절 문구가 필요하면 여기서 뽑아.</div>
                </Card>
                ...
                >
                  <div style={{ fontSize: 32, marginBottom: 8 }}>📤</div>
                  <div className="h2" style={{ margin: 0 }}>공유 카드</div>
                  <div className="hint" style={{ marginTop: 4 }}>익명화된 카드로 스토리에 올려.</div>
                </Card>
              </div>
              ...
              <div className="row" style={{ justifyContent: 'space-between', marginTop: 14 }}>
                ...
                <Button appearance="primary" onClick={() => { completeOnboarding(dispatch); dispatch({ type: 'SET_TAB', tab: 'dashboard' }) }}>
                  대시보드로 시작
                </Button>
              </div>
            </div>
